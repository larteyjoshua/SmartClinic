...
<head>
  ...
  <title>SmartClinic Gynecological</title>
...
$query = "SELECT app_id,fname, lname, app_message,app_date, TIME_FORMAT(app_time, '%h:%i %p')app_time,pNum FROM appointment WHERE department='Gyaenacology'ORDER BY app_date ASC, app_time ASC";
    ...
    while ($row = $result->fetch_assoc()) {
       $app_id = $row["app_id"];
        ...
        echo '<tr> 
                  ...
                  <td>'.$app_time.'</td>
                  <td>'.$pNum.'</td>
                  <td><a href="delete.php?app_id='. $app_id . '&page=5" class="btn btn-success" role="button">Served</a></td> 
              </tr>';
    }
    $result->free();
} 
